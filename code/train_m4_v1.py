...
class PoolerClassifier(nn.Module):
    ...
    def __init__(self, config, dropout=0.1):
        ...
        self.pooler1 = nn.Linear(config.hidden_size, config.hidden_size)
        self.pooler2 = nn.Linear(config.hidden_size, config.hidden_size)
        self.batchnorm = nn.BatchNorm1d(config.hidden_size)
        self.active = nn.Tanh()
        self.classifier = nn.Linear(config.hidden_size*2, 3)

    def forward(self, hidden_state, mask):
        """mask shape: B x Seq X 1"""
        pooled_output1 = self.pooler1(hidden_state)
        pooled_output1 = self.active(pooled_output1)
        maxpool_out, _ = (pooled_output1[:, 1:, :] * mask[:, 1:] - (1 - mask[:, 1:])*1e30).max(1)
        pooled_output2 = self.pooler2(hidden_state[:, 0, :])
        logits = self.classifier(self.dropout(torch.cat([pooled_output2, maxpool_out], dim=-1)))
        return logits
...
class BertClassifier(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
        ...
        logits = self.classifier(hidden_state, attention_mask[:, :, None].float())
