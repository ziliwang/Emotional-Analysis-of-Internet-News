...
def preprocess(text_file, label_file, tokenizer, max_length, input_pattern, clean_method,
    ...
    text_df.loc[pd.isna(text_df['content']), 'content'] = text_df[pd.isna(text_df['content'])]['title'].values
    if clean_method == 'space':
        space_reg = re.compile(r'\s+')
        text_df['content'] = text_df['content'].apply(lambda x: space_reg.sub('', x.lower().replace('\\n', '')))
        text_df['title'] = text_df['title'].apply(lambda x: space_reg.sub('', x.lower().replace('\\n', '')))
    elif clean_method == 'space+notzh':
        han = re.compile(r'[^\u4e00-\u9fff\,，。！？”“]{10,}')
        space_reg = re.compile(r'\s+')
        text_df['content'] = text_df['content'].apply(lambda x: space_reg.sub('', han.sub('', x.lower().replace('\\n', ''))))
        text_df['title'] = text_df['title'].apply(lambda x: space_reg.sub('', han.sub('', x.lower().replace('\\n', ''))))
    output = []
...
class BertClassifier(nn.Module):
    ...
    def __init__(self, bert, dropout=0.1):
        super().__init__()
        self.bert = bert
        self.classifier = PoolerClassifier(bert.config, dropout)
...
def training(model, optimizer, lr_scheduler, train_iter, val_iter, total_step, eval_every, early_stop,
             fold_idx, tokenizer, usegpu, logger):
    best_score = 0
    curr_step = 0
    cum_loss = 0
    cum_step = 0
    ...
    t = tqdm(total=total_step, desc=f'training fold {fold_idx}')
...
@click.option('--n-fold', type=int, default=5)
...
def main(log_in_file, lm_path, lm_type, data_path, usegpu,  n_fold, total_step, eval_every, early_stop, lr, weight_decay,
         lr_decay_in_layers, wd_decay_in_layers, max_length, max_title_rate, content_head_rate, batch_size, lr_scheduler_type,
         input_pattern, clean_method, warmup_rate, classifier_dropout, classifier_active, seed):
    ...
    test_iter = get_data_iter(processed_test, batch_size*4, collect_test_func, shuffle=False)
    pred = np.zeros((len(processed_test), 3))
    val_scores = []

    for fold_idx, (train_idx, val_idx) in enumerate(KFold(n_splits=n_fold, shuffle=True,
        ...
        if lr_scheduler_type == 'linear':
            ...
            raise ValueError
        train_iter = get_data_iter([processed_train[i] for i in train_idx], batch_size, collect_func)
        val_iter = get_data_iter([processed_train[i] for i in val_idx], batch_size*4, collect_func, shuffle=False)

        best_model, best_score = training(
