...
def epoch_train(model, optimizer, lr_scheduler, d_iter, usegpu, epoch):
    ...
    for input, target in tqdm(d_iter, f'epoch {epoch:02} training'):
        token_type = torch.tensor([0] + [0 for i in range(T_N+1)] + [1 for i in range(C_H+1)] +
                                  [1 for i in range(C_T+1)], dtype=input.dtype)
        if usegpu:
            input = input.cuda()
            target = target.cuda()
            token_type = token_type.cuda()
        token_type = token_type[None, :].expand(input.shape[0], -1)
        loss = model.forward(input_ids=input, attention_mask=input != PAD, token_type_ids=token_type, labels=target)[0]
        cum_step += 1
    ...
    with torch.no_grad():
        for input, target in tqdm(d_iter, f'epoch {epoch:02} training'):
            ...
            if usegpu:
                ...
                token_type = token_type.cuda()
            token_type = token_type[None, :].expand(input.shape[0], -1)
            logits = model.forward(input_ids=input, attention_mask=input != PAD, token_type_ids=token_type)[0]
