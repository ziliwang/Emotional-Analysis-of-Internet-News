...
def preprocess(text_file, label_file, tokenizer):
    ...
    for i in tqdm(range(text_df.shape[0]), 'preprocess'):
        ...
        if label_file and (e_id not in id2lab or (len(e_t) + len(e_c)) == 0):
            continue
        e_c_t = tokenizer.encode(e_c)
        e_t_t = tokenizer.encode(e_t)
        p_t = pad_token(e_t_t, T_N)
        p_ch = pad_token(e_c_t, C_H)
        p_ct = pad_token(e_c_t[::-1], C_T)[::-1]
        output.append({
            'input': [CLS] + [SEP] + p_t + [SEP] + p_ch + [SEP] + p_ct,
            'target': id2lab[e_id] if label_file else 0,
            'id': e_id,
        })
    return output
...
def predict(model, d_iter, usegpu):
    ...
    pred = []
    with torch.no_grad():
        for input in tqdm(d_iter, f'predict'):
            token_type = torch.tensor([0] + [1 for i in range(T_N+1)] + [2 for i in range(C_H+1)], dtype=input.dtype)
            if usegpu:
                input = input.cuda()
                token_type = token_type.cuda()
...
def training(model, optimizer, lr_scheduler, train_iter, val_iter, epoch,
    ...
    for e in range(epoch):
        ...
        if epoch_score > best_score:
            ...
            best_model = deepcopy(model.state_dict())
            best_epoch = e
    return best_score, best_model, best_epoch


def n_fold(train_data, test_data, model, model_s, epoch,
           seed, logger, usegpu, k_fold=5, batch_size=16):
    test_iter = get_data_iter(test_data, batch_size*2, collect_test_func, shuffle=False)
    ...
    for fold_n, (train_idx, val_idx) in enumerate(
            KFold(n_splits=k_fold, shuffle=True, random_state=seed).split(train_data)):
        ...
        optimizer = AdamW(
            ...
            [{'params': [p for n, p in model.named_parameters()
             ...
             for i, layer_name in enumerate(bert_layer_names[::-1])])
        train_iter = get_data_iter([train_data[i] for i in train_idx], batch_size, collect_func)
        val_iter = get_data_iter([train_data[i] for i in val_idx], batch_size*2, collect_func, shuffle=False)
        t_step = epoch*len(train_iter)
        lr_scheduler = WarmupLinearSchedule(optimizer, warmup_steps=0.1*t_step,
                                            t_total=t_step)
        best_score, best_model_s, best_epoch = training(
                    model=model, optimizer=optimizer, lr_scheduler=lr_scheduler,
                    train_iter=train_iter, val_iter=val_iter, epoch=epoch,
                    tokenizer=tokenizer, usegpu=usegpu, logger=logger)
        logger.info(f'fold: {fold_n} - best_epoch: {best_epoch:02} - best score: {best_score:.4f}')
        model.load_state_dict(best_model_s)
        pred += predict(model, test_iter, usegpu)
    submit = pd.DataFrame()
    submit['id'] = [i['id'] for i in test_data]
...
def main(log, mpath, usegpu, epoch):
    ...
    processed_train = preprocess('../data/Train_DataSet.csv', '../data/Train_DataSet_Label.csv', tokenizer)
    processed_test = preprocess('../data/Test_DataSet.csv', False, tokenizer)
    SEED = 23
    ...
    n_fold(processed_train, processed_test, model, model_s, epoch, SEED, logger, usegpu)
