...
class PoolerClassifier(nn.Module):
    ...
    def forward(self, hidden_state, mask):
        ...
        pooled_output = self.active(pooled_output)
        maxpool_out, _ = (pooled_output[:, 1:, :] * mask[:, 1:] - (1 - mask[:, 1:])*1e30).max(1)
        logits = self.classifier(self.dropout(torch.cat([pooled_output[:, 0, :], maxpool_out], dim=-1)))
        return logits
